namespace :iban do
  ...
  task generate: :environment do
    user_without_iban = 0
    user_with_valid_bank_account_number = 0
    ...
    User.all.each do |user|
      if user.bank_code? && user.bank_account_number? && !user.iban? && !user.bic?
        ...
        if !bank_info[:iban].blank? && !bank_info[:bic].blank? && bank_info[:return_code] == '00'
          user_with_valid_bank_account_number += 1
          # puts bank_info[:iban]
          # puts bank_info[:bic]
    ...
    puts 'User without iban and correct kntNr and Blz: ' + user_with_valid_bank_account_number.to_s
  end
end
