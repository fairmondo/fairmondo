class Exhibit < ActiveRecord::Base
  ...
  private
    ...
    def self.fill_exhibits exhibits, queue, count
      ...
      exhibits + Exhibit.where(queue: queue).where("exhibits.id NOT IN (?)", exhibits.map{|e| e.id }).older_exhibited.article_active.limit(filler_count) unless filler_count == 0
    end
end
