...
module BusinessTransactionMailerHelper
  ...
  def article_payment_info transaction, role
    ...
    if role == :buyer && !transaction.article_seller_ngo
      if show_fair_percent_for? transaction.article
      ...
      if show_friendly_percent_for? transaction.article
        ngo = transaction.article_friendly_percent_organisation
    ...
    def transport_details transaction
      ...
      if ['pickup', 'type1', 'type2'].include? transaction.selected_transport
        ...
        output += case transaction.selected_transport
          when 'pickup'
            t('enumerize.business_transaction.selected_transport.pickup') + "\n"
          when 'type1'
