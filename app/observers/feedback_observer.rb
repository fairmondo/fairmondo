  ...
  def after_save(feedback)
    ...
    case feedback.variety
      when "report_article" then
        ArticleMailer.delay.report_article( feedback.article, feedback.user, feedback.text )
      when "send_feedback" then
        feedback.subject.prepend("[Feedback] ")
        FeedbackMailer.delay.feedback_and_help( feedback, feedback.feedback_subject )
      when "get_help" then
