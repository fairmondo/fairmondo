...
* Branch management: "master" is the currently deployed version. New features are developed on the branch "develop" or feature branches that branch from develop. Bugfixes are currently done on "release-lizzie". Please send merge requests to the appropriate branches (develop or release-lizzie).
